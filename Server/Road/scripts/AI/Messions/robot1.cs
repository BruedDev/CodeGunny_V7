...
{
    public class robot1 : AMissionControl
    {
        private List<SimpleNpc> someNpc = new List<SimpleNpc>();

        private int dieRedCount = 0;

        private int[] npcIDs = { 80000, 80001 };
        ...
        {
            base.OnStartGame();
            Game.IsBossWar = "100000";
            //左边五只小怪
            int index = Game.Random.Next(0, npcIDs.Length);
            someNpc.Add(Game.CreateNpc(npcIDs[index], 52, 506, 1, 1));
            index = Game.Random.Next(0, npcIDs.Length);
            someNpc.Add(Game.CreateNpc(npcIDs[index], 100, 507, 1, 1));
            index = Game.Random.Next(0, npcIDs.Length);
            someNpc.Add(Game.CreateNpc(npcIDs[index], 155, 508, 1, 1));
            index = Game.Random.Next(0, npcIDs.Length);
            someNpc.Add(Game.CreateNpc(npcIDs[index], 210, 507, 1, 1));
            index = Game.Random.Next(0, npcIDs.Length);
            someNpc.Add(Game.CreateNpc(npcIDs[index], 253, 507, 1, 1));

            //右边五只小怪
            index = Game.Random.Next(0, npcIDs.Length);
            someNpc.Add(Game.CreateNpc(npcIDs[index], 1275, 508, 1, -1));
            index = Game.Random.Next(0, npcIDs.Length);
            someNpc.Add(Game.CreateNpc(npcIDs[index], 1325, 506, 1, -1));
            index = Game.Random.Next(0, npcIDs.Length);
            someNpc.Add(Game.CreateNpc(npcIDs[index], 1360, 508, 1, -1));
            index = Game.Random.Next(0, npcIDs.Length);
            someNpc.Add(Game.CreateNpc(npcIDs[index], 1410, 506, 1, -1));
            index = Game.Random.Next(0, npcIDs.Length);
            someNpc.Add(Game.CreateNpc(npcIDs[index], 1475, 508, 1, -1));
        }
