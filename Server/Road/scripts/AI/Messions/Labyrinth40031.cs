...
{
    public class Labyrinth40031 : AMissionControl
    {
        private List<SimpleNpc> someNpc = new List<SimpleNpc>();

        private int kill = 0;

        private int npcIDs = 40051;

        private int[] birthX = { 910, 890, 860, 840, 820, 400, 380, 360, 340, 320 };
        ...
        public override void OnStartGame()
        {
            ...
            for (int i = 0; i < birthX.Length; i++)
            {
                if (birthX[i] > 800)
                {
                    someNpc.Add(Game.CreateNpc(npcIDs, birthX[i], 770, 1, -1));
                }
                else
                {
                    someNpc.Add(Game.CreateNpc(npcIDs, birthX[i], 770, 1, 1));
