...
namespace GameServerScript.AI.NPC
{
    public class GoblinFirst : ABrain
    {
        ...
        {
            m_wallRight = ((PVEGame)Game).CreatePhysicalObj(1460, 580, "wallLeft", "", "1", 1, 0);
            m_wallRight.SetRect(-75, -159, 53, 130);
		...
		private void GoAttack()
        {
			...
			m_moive = ((PVEGame)Game).Createlayer(target.X, target.Y, "wallLeft", "asset.game.4.xiaopao", "1", 1, 0);
			Body.Shoot(174, target.X, target.Y, 1, 1, 1, 0);
			Body.RangeAttacking(target.X + 20, target.X - 20, "cry", 500, null);
		}
