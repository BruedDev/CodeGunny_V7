            ...
            if (dis > 97)
            {
                ...
                if (player.Y < 420 && player.X < 210)
                {
                    if (Body.Y > 420)
                    {
                        if (Body.X - dis < 50)
                        {
							//Body.FlyTo(Body.X, Body.Y, 25, Body.Y, "fly", 1200, 8, new LivingCallBack(Jump));
                        }
                        else
                        {
							//Body.FlyTo(Body.X, Body.Y, Body.X - dis, Body.Y, "fly", 1200, 8, new LivingCallBack(MoveBeat));
                        }
                    }
                    else
                    {
                ...
                else
                {
                    if (Body.Y < 420)
                    {
                        ...
                        //{
                        //    Body.FallFrom(Body.X, Body.Y + 240, null, 0, 0, 12, 8, new LivingCallBack(FallBeat));
                        //}
                        //else
                        //{
                        //    Body.MoveToSpeed(Body.X + dis, Body.Y, "fly", 1200, 8, new LivingCallBack(MoveBeat));
                        //}
                        if (Body.X + dis > 200)
                        {
							//Body.FlyTo(Body.X, Body.Y, 200, Body.Y, "fly", 1200, 8, new LivingCallBack(Fall));
                        }
                    }
                    else
                    {
            ...
            if (sayCount > 0)
            {
                ...
                for (int i = 0; i < sayCount;)
                {
                    ...
                    if (livings[index].IsSay == false)
                    {
                        ...
                        //livings[index].Say(FiveTerrorThirdNpc.GetOneChat(), 0, delay);
                        i++;
                    }
                }
            }
