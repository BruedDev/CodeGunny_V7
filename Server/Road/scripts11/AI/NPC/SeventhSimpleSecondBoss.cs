    ...
    public class SeventhSimpleSecondBoss : ABrain
    {
        ...
        private void AttackingC()
        {
            ...
            if (target != null)
            {
                ...
                if (Body.ShootPoint(mtX, target.Y, 83, 1000, 10000, 1, 1, 3300))
                {
		...
		private void GoMovie()
        {
            ...
            {
                int mtX = target.X;                
                Body.RangeAttacking(mtX - 200, mtX + 200, "cry", 1000, null);
                moive = ((PVEGame)Game).Createlayer(mtX, target.Y, "moive", "asset.game.seven.choud", "out", 1, 0);
                Body.CallFuction(new LivingCallBack(GoAttacking), 2000);
            }
		}
		...
		private void GoAttacking()
        {
			...
			if (!IsEixt && target != null)
            {
                ...
                foreach (Player player in players)
                {
                    ...
                    {
                        player.AddEffect(new ContinueReduceBlood(2, 200, player), 0);
                    }
                }
            }
