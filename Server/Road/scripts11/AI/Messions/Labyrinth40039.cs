...
{
    public class Labyrinth40039 : AMissionControl
    {
        private List<SimpleNpc> someNpc = new List<SimpleNpc>();

        private int kill = 0;

        private int npcIDs = 40064;

        private int npcIDs2 = 40063;

        private int[] birthX = { 1210, 1190, 1160, 1140, 1120, 300, 280, 260, 240, 220 };
        ...
        public override void OnPrepareNewSession()
        {
            ...
            int[] resources = { npcIDs, npcIDs2 };
            int[] gameOverResources = { npcIDs, npcIDs2 };
        ...
        public override void OnStartGame()
        {
            ...
            for (int i = 0; i < birthX.Length; i++)
            {
                if (birthX[i] > 1000)
                {
                    someNpc.Add(Game.CreateNpc(npcIDs, birthX[i], 171, 1, -1, config));
                }
                else
                {
                    someNpc.Add(Game.CreateNpc(npcIDs2, birthX[i], 171, 1, 1));
                }
            }
